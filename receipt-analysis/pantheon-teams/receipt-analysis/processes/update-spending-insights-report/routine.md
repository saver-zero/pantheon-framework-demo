...
# Routine: update-spending-insights-report {% if pantheon_sections is defined %}--sections {{ _pantheon_sections_string }}{% endif %}

**Objective:** To design and update spending-insights-report {% if pantheon_sections is defined %}sections: {{ _pantheon_sections_string }}{% endif %}
...
Step {{ _current_step_index.num }}. **Get Schema:** Retrieve the structural contract for the spending-insights-report{% if pantheon_sections is defined %} sections: {{ _pantheon_sections_string }}{% endif %}. Use `pantheon get schema update-spending-insights-report{{ _pantheon_sections_param }} --actor <your_agent_name>`.
{% set _current_step_index.num = _current_step_index.num + 1 %}

Step {{ _current_step_index.num }}. **Get Context** Retrieve the context for the spending-insights-report. Use `pantheon execute get-spending-insights-report --sections context,{{ initial_section }} --actor <your_agent_name> --id <spending-insights-report id>`
{% set _current_step_index.num = _current_step_index.num + 1 %}
...
Step {{ _current_step_index.num }}. **High-Level Design:** Define the core content of the spending-insights-report{% if pantheon_sections is defined %} sections: {{ _pantheon_sections_string }}{% endif %} using the schema. Follow each field's authoring_guidance so every update stays concise and high leverage.
...
Step {{ _current_step_index.num }}. **Get temp file location:** Get the temp file location. Use `pantheon get tempfile --process update-spending-insights-report --actor <your_agent_name>`.

{% set _current_step_index.num = _current_step_index.num + 1 %}

Step {{ _current_step_index.num }}. **Save the JSON:** Write the content of the spending-insights-report{% if pantheon_sections is defined %} sections: {{ _pantheon_sections_string }}{% endif %} designed in the previous steps into a single, valid JSON file, writing it to the <tempfile>.

{% set _current_step_index.num = _current_step_index.num + 1 %}

Step {{ _current_step_index.num }} (finish). **Execute Process:** Execute the process to update spending-insights-report{% if pantheon_sections is defined %} sections: {{ _pantheon_sections_string }}{% endif %} with the fully assembled <tempfile>. Use `pantheon execute update-spending-insights-report{{ _pantheon_sections_param }} --from-file "<tempfile>" --id <spending-insights-report id> --actor <your_agent_name>{{ _insert_mode }}`. Having quotes around <tempfile> is critical to prevent any shell parsing issues. You are now done. Stop. Do not do anything else for this routine. Do not search, list, verify anything else for this routine.