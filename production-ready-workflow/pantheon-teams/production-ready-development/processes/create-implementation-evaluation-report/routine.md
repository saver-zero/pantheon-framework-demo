# Routine: create-implementation-evaluation-report

**Objective:** To design and create a complete implementation-evaluation-report.
...
Step 1. **Get Schema:** Retrieve the structural contract for the implementation-evaluation-report. Use `pantheon get schema create-implementation-evaluation-report --actor <your_agent_name>`.
...
Step 3. **High-Level Design:** Define the core content of the implementation-evaluation-report using the schema. Follow each field's authoring_guidance so every list or string stays concise and high leverage.
...
Step 5. **Get temp file location:** Get the temp file location. Use `pantheon get tempfile --process create-implementation-evaluation-report --actor <your_agent_name>`.

Step 6. **Save the JSON:** Write the content of the implementation-evaluation-report designed in the previous steps into a single, valid JSON file, writing it to the <tempfile>.

Step 7 (finish). **Execute Process:** Execute the process to create the implementation-evaluation-report with the fully assembled <tempfile>. Use `pantheon execute create-implementation-evaluation-report --from-file "<tempfile>" --actor <your_agent_name>`. Having quotes around <tempfile> is critical to prevent any shell parsing issues. You are now done. Stop. Do not do anything else for this routine. Do not search, list, verify anything else for this routine.
